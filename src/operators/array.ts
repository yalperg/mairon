...
export function registerArrayOperators(): void {
  registerOperator('includes', (fieldValue: unknown, condition: SimpleCondition): boolean => {
  ...
  registerOperator('excludes', (fieldValue: unknown, condition: SimpleCondition): boolean => {
    if (!isArray(fieldValue)) {
      return false;
    }
    return !fieldValue.includes(condition.value);
  ...
  registerOperator('includesAll', (fieldValue: unknown, condition: SimpleCondition): boolean => {
  ...
  registerOperator('includesAny', (fieldValue: unknown, condition: SimpleCondition): boolean => {
    if (!isArray(fieldValue) || !isArray(condition.value)) {
      return false;
    }
